...
async function install (fs, gyp, argv) {
  log.warn('starting installation')
  ...
  async function go () {
    ...
    if (tarPath) {
      ...
      try {
        ...
        log.warn('extracting tar 2', res.body.length)
        await streamPipeline(
          ...
          new ShaSum((_, checksum) => {
            ...
            log.warn('filename', filename, 'devDir', devDir, 'tarballUrl', release.tarballUrl)
          }),
          tar.extract({
            strip: 1,
            cwd: devDir,
